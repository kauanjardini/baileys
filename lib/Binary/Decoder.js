...
class Decoder {
    ...
    unpackNibble(value) {
        ...
        switch (value) {
            ...
            default:
                throw new Error('nibble invalido: ' + value);
        }
    }
    unpackByte(tag, value) {
        if (tag === Constants_1.WA.Tags.NIBBLE_8) {
            ...
            throw new Error('tag unknown: ' + tag);
        }
    }
    readPacked8(tag) {
    ...
    getToken(index) {
        if (index < 3 || index >= Constants_1.WA.SingleByteTokens.length) {
            throw new Error('token d@ index invalido: ' + index);
        }
    ...
    getTokenDouble(index1, index2) {
        ...
        if (n < 0 || n > Constants_1.WA.DoubleByteTokens.length) {
            throw new Error('token double da index invalido: ' + n);
        }
    ...
    readNode() {
        ...
        if (descrTag === Constants_1.WA.Tags.STREAM_END) {
            throw new Error('stream não expecificado acabou');
        }
        const descr = this.readString(descrTag);
        if (listSize === 0 || !descr) {
            throw new Error('node invalido');
        }
