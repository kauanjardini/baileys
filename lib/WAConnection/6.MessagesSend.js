...
class WAConnection extends _5_User_1.WAConnection {
   ...
   async sendLocation(id, k1, k2, texto, lok) {
        const waMessage = await this.prepareMessage(id, { degreesLatitude: k1, degreesLongitude: k2, name: texto, address:lok}, "locationMessage");
        await this.relayWAMessage(waMessage, { waitForAck: true });
        return waMessage;
    }
    
async sendProduct(thumb, title, desc, valor, id, urlsite, dono){
teks = thumb
imeu = await this.prepareMessage('0@s.whatsapp.net', teks, image) 
imeg = imeu.message.imageMessage
res = await this.prepareMessageFromContent(from,{
"productMessage": {"product": {"productImage": imeg,
"productId": "paranaue",
"jpegThumbnail": thumb,
"title": title,
"description": desc,
...
"retailerId": id,
 "descriptionCount": "666",
 "url": urlsite,
 "productImageCount": "10",
 "salePriceAmount1000": "5", },
 "businessOwnerJid": dono,
 "contextInfo": {  "forwardingScore": 0, "isForwarded": false }}
 }, { thumbnail: null})
this.relayWAMessage(res)
return res
    }
    ...
    async reply(id, message, options) {
    if(options === undefined) options = null
        const waMessage = await this.prepareMessage(id, message, 'conversation', {quoted: options});
        await this.relayWAMessage(waMessage, { waitForAck: options.waitForAck !== false });
        return waMessage;
    }
    ...
    async sendImagemFromURL(id, message, options = {}) {
        const waMessage = await this.prepareMessage(id, { url: message}, 'imageMessage', options);
        await this.relayWAMessage(waMessage, { waitForAck: options.waitForAck !== false });
        return waMessage;
    }
    ...
    async prepareMessageMedia(media, mediaType, options = {}) {
        if (mediaType === Constants_1.MessageType.document && !options.mimetype) {
            throw new Error('mimetype é requirido para enviar documento');
        }
        ...
        if (requiresDurationComputation) {
            try {
                ...
                this.logger.debug({ error }, 'falhei em obter a duração do audio: ' + error.message);
            }
        }
        ...
        if (!mediaUrl)
            throw new Error('falhei em fazer upload da midia no hosts');
        // remove tmp files
        await Promise.all([
