    ...
    async connect() {
        ...
        while (this.state === 'connecting') {
            ...
            try {
                ...
                this.logger.warn({ error }, `conecção mal-sucedida n•: ${tries} falhado por: ${error}${willReconnect ? ', tentando novamente...' : ''}`);
        ...
        this.logger.info('abrindo conecção com o WhatsApp Web');
    ...
    async connectInternal(options, delayMs) {
        ...
        const connect = () => new Promise((resolve, reject) => {
            ...
            this.conn.once('open', async () => {
                ...
                this.logger.info(`conectado ao servidor do WhatsApp Web, autenticação via ${reconnectID ? 'reconnect' : 'takeover'}`);
                try {
    ...
    onMessageRecieved(message) {
        ...
        if (message[0] === '!') {
            ...
            try {
                ...
                this.logger.error({ error }, `erro encontrado no descriptogramendo da mensagem, causador: ${error}`);
