...
exports.Browsers = {
    ...
    baileys: browser => ['Edicth-bot', browser, '3.0'],
...
const WA_MESSAGE_ID = (m) => exports.GET_MESSAGE_ID(m.key||'555');
...
function decryptWA(message, macKey, encKey, decoder, fromMe = false) {
    ...
    if (commaIndex < 0)
        throw new Constants_1.BaileysError('mensagem invalida', { message }); // if there was no comma, then this message must be not be valid
        ...
        if (typeof data === 'string') {
            ...
            */
            ...
            if (checksum.equals(computedChecksum)) {
                ...
                throw new Constants_1.BaileysError('checcap falhado', {
...
const getGotStream = async (url, options = {}) => {
    ...
    await new Promise((resolve, reject) => {
        ...
        fetched.once('response', ({ statusCode: status }) => {
            if (status >= 400) {
                reject(new Constants_1.BaileysError('codigo invalido (' + status + ') retornado', { status }));
            }
            else {
                resolve(undefined);
            }
...
async function decryptMediaMessageBuffer(message) {
    ...
    */
    ...
    if (message.productMessage) {
        ...
        if (!product)
            throw new Constants_1.BaileysError('produto sem imagem', message);
