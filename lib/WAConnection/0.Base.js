...
class WAConnection extends events_1.EventEmitter {
    constructor() {
        ...
        this.browserDescription = Utils.Browsers.baileys('DEV');
        ...
        this.logger = logger.child({ erro: 'Baileys' });
        /** log messages */
    ...
    loadAuthInfo(authInfo) {
        ...
        if (typeof authInfo === 'string') {
            this.logger.info(`carregando a autenticação das credenciais do ${authInfo}`);
    ...
    exitQueryIfResponseNotExpected(tag, cancel) {
        ...
        const listener = ({ connected }) => {
            if (connected) {
                timeout = setTimeout(() => {
                    this.logger.info({ tag }, `cancelando o aguardo das mensagens, a uma resposta que é inesplicavel em seu telefone`);
                    cancel({ reason: 'A uma resposta não explicada', status: 422 });
    ...
    startPhoneCheckInterval() {
        ...
        if (!this.phoneCheckInterval) {
            ...
            this.phoneCheckInterval = setInterval(() => {
                ...
                this.logger.info('checando a conecção do seu telefone...');
                this.sendAdminTest();
    ...
    closeInternal(reason, isReconnecting = false) {
        this.logger.info(`conecção fechada , por ${reason}${isReconnecting ? ', reconectando em alguns segundos...' : ''}`);
        this.state = 'close';
        this.phoneConnected = false;
