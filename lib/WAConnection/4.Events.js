...
class WAConnection extends _3_Connect_1.WAConnection {
    constructor() {
        ...
        this.assertChatGet = jid => {
            ...
            if (!chat)
                throw new Error(`chat '${jid}' não encontrado`);
            return chat;
        };
        ...
        this.chatsDebounceTimeout.setTask(() => {
            this.logger.debug('vendo o chat pesquisado');
        ...
        this.on('open', () => {
            ...
            this.logger.debug('enviando as requerições');
        });
        // on disconnects
        ...
        this.on('CB:Pong', json => {
            if (!json[1]) {
                ...
                this.logger.info('Acabou a conecção no telefone, porisso eu estou fechando...');
            }
        ...
        this.on('CB:response,type:chat', json => {
            ...
            json[2].forEach(([item, chat]) => {
                if (!chat) {
                    this.logger.warn(`vendo explicação de chat null: ${item}`, chat);
                    return;
                }
            ...
            this.logger.info(`recebendo ${json[2].length} chats`);
        ...
        const onLastBatchOfDataReceived = () => {
            ...
            const chatsWithMissingMessages = Object.keys(overlaps).map(jid => {
                ...
                if (!overlaps[jid].didOverlap && overlaps[jid].requiresOverlap) {
                    this.logger.debug(`recebendo mensagens de ${jid}, não foi possivel encontrar a previsão das mensagens,por isso estou limpando...`);
                    const chat = this.chats.get(jid);
                    if (chat) {
            ...
            if (messages) {
                ...
                messages.reverse().forEach(([, , message]) => {
                    ...
                    if (chat) {
                        ...
                        this.logger.debug({ jid }, `chat não encontrado`);
                });
        ...
        this.on('CB:response,type:contacts', json => {
            ...
            json[2].forEach(([type, contact]) => {
                if (!contact)
                    return this.logger.info(`inesplicação de contato null : ${type}`, contact);
            ...
            updatedChats.length > 0 && this.emit('chats-update', updatedChats);
            this.logger.info(`recebendo ${json[2].length} contatos`);
        ...
        this.on('CB:action,,read', async (json) => {
            ...
            if (chat) {
                ...
                this.logger.warn('recebendo atualizaçào de chat unknouw ' + jid);
            }
        });
        this.on('qr', qr => {
    ...
    onMessageStatusUpdate(jid, key, status) {
        ...
        if (msg) {
            if (typeof status !== 'undefined') {
                ...
                this.logger.warn({ update: status }, 'recebendo mensagen de atualização de status');
            }
        }
        else {
            this.logger.debug({ unhandled: true, update: status, key }, 'recebendo mensagen de status update e não está presende na message');
        }
    }
    contactAddOrGet(jid) {
            ...
            switch (protocolMessage.type) {
                case Constants_1.WAMessageProto.ProtocolMessage.ProtocolMessageType.REVOKE:
                    ...
                    if (found === null || found === void 0 ? void 0 : found.message) {
                        this.logger.info('deletando mensagem: ' + protocolMessage.key.id + ' no chat: ' + protocolMessage.key.remoteJid);
